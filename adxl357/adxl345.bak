...
int main(int argc, char *argv[]) {
    ...
    h1 = spiOpen(1, speedSPI, 3);
    if (h1 < 0){
        printf("H1 Open error");
        return 1;
    }
    // Set the Sampling Rate to 3200 Hz/ BW of 1600Hz (0x0F)
    data[0] = BW_RATE;
    data[1] = 0x0F;
    writeBytes(h, data, 2);
    // Set the data resoultion to 13 bits (+-16g)
    data[0] = DATA_FORMAT;
    ...
    // therefore, saved data can be easily analyzed (e. g. with fft)
       ...
       double *r2x, *r2y, *r2z;
        r2x = malloc(samplesMaxSPI * sizeof(double));
        r2y = malloc(samplesMaxSPI * sizeof(double));
        r2z = malloc(samplesMaxSPI * sizeof(double));
        ...
        //2nd acc
        double *a2x, *a2y, *a2z;
        a2x = malloc(samples * sizeof(double));
        a2y = malloc(samples * sizeof(double));
        a2z = malloc(samples * sizeof(double));
        ...
        for (i = 0; i < samplesMaxSPI; i++) {
            ...
            data1[0] = DATAX0;
            bytes = readBytes(h1, data1, 7);
            if (bytes == 7) {
                x = (data1[2]<<8)|data1[1];
                y = (data1[4]<<8)|data1[3];
                z = (data1[6]<<8)|data1[5];
                r2x[i] = x * accConversion;
                r2y[i] = y * accConversion;
                r2z[i] = z * accConversion;
        ...
        for (i = 0; i < samples; i++) {
            fprintf(f, "%.5f, %.5f, %.5f, %.5f, %.5f, %.5f, %.5f  \n", at[i], a2x[i], a2y[i], a2z[i], ax[i], ay[i], az[i]);
        }
        fclose(f);
    }
